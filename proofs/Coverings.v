...
Section Coverings.
  ...
  (** THEOREM 6. A positive obligation is name-agnostic and vocabulary-free: its verdict is an
      equality between two data and mentions no program, no pattern, and no word. There is
      nothing in it for a rename or a new runner to change -- which is exactly the obligation
      Theorem 5 said a nominal guard can only ever manage. *)
  Theorem positive_needs_no_vocabulary :
    forall c, positive c <-> exists d, claimed c = Some d /\ observed c = Some d.
  Proof. intros c. split; intro H; exact H. Qed.
