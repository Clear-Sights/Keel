    ...
    def test_stop_retires_files_created_and_deleted_in_one_bash_act(self):
        ...
        for index, removed in enumerate((None, [], ["other.txt"])):
            with self.subTest(files_removed=removed):
                ...
                # A relative subject has no stored origin; later removal cannot resolve it.
        ...
        # Supply both observations with an unambiguous absolute target at the host boundary.
        target = str(self.cwd / "fleeting.txt")
        dispatch.post_tool_use(self.table, self.ledger, {
            **event, "hook_event_name": "PostToolUse",
            "keel_effect": record(files_changed=[target], files_removed=[target])})
        ...
        original = self.ledger.path.read_bytes()
        self.assertEqual({}, dispatch.reconcile(self.table, self.ledger, stop))
        ...
        self.assertEqual(notes, repeated[:len(notes)])
        self.assertEqual(rows, [note["demand"] for note in repeated[len(notes):]
                                if note["kind"] == "retired_missing"])
        self.assertEqual(original, self.ledger.path.read_bytes())
        (self.cwd / "fleeting.txt").write_text("the target returned\n")
        self.assertEqual("block", dispatch.reconcile(self.table, self.ledger, stop)["decision"])
        self.assertEqual(rows, self.ledger.open_demands("recovery", ""))
        for row in rows:
            self.assertFalse(self.ledger.is_licensed("recovery", "", row["id"]))
